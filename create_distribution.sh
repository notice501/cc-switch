...
mkdir -p "$DIST_DIR/src-tauri"
mkdir -p "$DIST_DIR/src/skills"
mkdir -p "$DIST_DIR/src/components"

echo "Copying backend files..."
cp src-tauri/src/task_dispatcher.rs "$DIST_DIR/src-tauri/" 2>/dev/null || echo "  WARNING: task_dispatcher.rs not found in expected location"
cp src-tauri/src/commands/task_dispatcher.rs "$DIST_DIR/src-tauri/commands/" 2>/dev/null || echo "  WARNING: commands/task_dispatcher.rs not found in expected location"
cp -r src/skills/task-dispatcher "$DIST_DIR/src/skills/" 2>/dev/null || echo "  WARNING: skills/task-dispatcher not found in expected location"

echo "Copying UI components..."
cp src/components/TaskDispatcher.tsx "$DIST_DIR/src/components/" 2>/dev/null || echo "  NOTE: TaskDispatcher.tsx not found (optional)"

echo "Copying configuration files..."
cp src-tauri/Cargo.toml "$DIST_DIR/src-tauri/" 2>/dev/null || echo "  WARNING: Cargo.toml not found in src-tauri/"
cp src-tauri/tauri.conf.json "$DIST_DIR/src-tauri/" 2>/dev/null || echo "  WARNING: tauri.conf.json not found in src-tauri/"

echo "Copying documentation..."
cp CC_SWITCH_2.0_PROVIDER_SPECIFIC_EDITION.md "$DIST_DIR/" 2>/dev/null || echo "  WARNING: Main documentation not found"
cp IMPLEMENTATION_SUMMARY.md "$DIST_DIR/" 2>/dev/null || echo "  WARNING: Implementation summary not found"
cp README.md "$DIST_DIR/" 2>/dev/null || echo "  NOTE: README.md not found (optional)"
