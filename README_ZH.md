...
下面这些是这个 fork 相比上游重点新增或重新设计的能力，也是当前 README 需要反映的主线。

### 1. Pro 独立实例与并存安装

- **CCswitch Pro** 使用独立 app identity、独立配置目录、独立 deep link scheme、独立本地存储前缀
- 标准版和 Pro 版可以并存安装，不再默认共用 `~/.cc-switch`
- Pro 默认数据目录是 `~/.ccswitch-pro`
- 适合把上游版当通用配置工具，把 Pro 版当多模型协作工作台

### 2. Claude alias 启动器

- 可以给 Claude provider 配 alias，自动生成类似 `claude-dou`、`claude-kimi` 这样的本地命令
- alias 不只是换名字，而是会切到对应 provider 的独立配置
- alias home 会隔离 `projects`、`sessions`、`history`、`debug` 等运行态，避免不同 provider 抢同一份 Claude 状态
- alias 启动器会自动带上合适的权限模式，默认继承为 `bypassPermissions`
- dispatch statusline 也会同步到这些 alias 环境里

### 3. 独立 dispatch 服务

- Claude Code 里的 `/dispatch-task` 现在不是本地脚本硬拼，而是会调用桌面端的 loopback dispatch service
- 支持按 `alias`、provider 名、provider id 或 `current` 精确选目标
- Claude 和 Codex 都可以作为 dispatch 目标
- Codex dispatch 会使用目标 provider 自己的 auth / config，而不是绑死默认账号

### 4. 后台异步 dispatch

- dispatch 默认后台运行，不再要求当前 Claude turn 同步阻塞等待
- 如需保留旧行为，可以显式加 `wait=true`
- 后端现在有独立的 run registry，而不是只记“最后一次状态”
- 支持查看多条 run、查看单条详情、查看历史、取消运行中任务

### 5. tmux bridge 模式

- `monitor=pane` 不再只是右侧看日志
- 在 **tmux** 里，它会真正 split 出一个新的 pane，在右侧启动一个真实 Codex 会话
- 子任务完成后，会把 callback 自动回送到左侧主 Claude pane
- 这更接近“主 agent + 子 agent 同时在线协作”的工作流，而不是一个无头子进程加监控窗口

### 6. Dispatch 可观测性增强

- 支持 `/dispatch-task list`、`show`、`watch`、`cancel`
- statusline 会显示当前 dispatch 是否在跑、最近一次结果是否成功
- run 完成后可回看 stdout / stderr / deliverable
- 应用重启后会回收陈旧的 running 状态，避免 run 永久卡死
...
- dispatch 默认异步后台运行，支持 run 列表、详情、取消与 tmux bridge
...
- Claude alias 额外使用独立 home 和独立运行态目录，减少 provider 之间的状态串扰
...
3. 如果你常用固定 provider，可以给 Claude provider 配 alias，直接通过 `claude-xxx` 启动
4. 需要更强模型做判断时，切到更强的 provider；需要更低成本执行时，把子任务 dispatch 给更便宜的 Claude / Codex provider
5. 如果你在 tmux 里工作，可以用 `monitor=pane` 把 Codex 子任务放到右侧真实 pane 中运行
6. 根据用量和成本面板，逐步形成适合自己的模型分工方式

> **注意**：首次启动可以手动导入现有 CLI 配置作为默认 provider。当前分发链路主要覆盖 Claude 和 Codex provider。

## 一个更准确的当前工作流

### 方式 A：直接切 provider / alias

- 你可以在 CCswitch Pro 里切当前 provider
- 也可以直接使用 `claude-dou`、`claude-kimi` 这类 alias 命令进入指定 Claude provider
- 这适合“我这轮会话从头到尾就想用某个 provider”的情况

### 方式 B：主会话 + 子任务分发

- 左边保留当前 Claude 主会话
- 遇到实现型、批量型、可拆分的任务时，用 `/dispatch-task` 发给另一个 Claude 或 Codex provider
- 如果你在 tmux 中，可以把 Codex 子任务开到右侧 pane，让两个 agent 同时工作

### 方式 C：强模型判断，弱模型执行

- 主会话用更强的 Claude provider 做需求边界、架构设计、任务拆解
- 子任务分发给更便宜的 Codex provider 做落地实现
- 最后再回到主会话做 review、收口与继续推进
...
- 如果使用 `monitor=pane`，目前要求目标是 **Codex provider**
...
5. 用 `/dispatch-task status`、`last`、`logs`、`list`、`show` 查看执行情况
6. 如果你在 tmux 里，可以用 `monitor=pane` 让 Codex 子任务直接跑在右侧 pane
...
- 现在默认会后台运行，不再阻塞当前 Claude Code 会话；如果你希望当前会话一直等到子任务结束，可以加 `wait=true`
- `monitor=pane` 目前只支持在 **tmux** 中给 **Codex target** 使用
- 真正的任务内容必须写在 `--` 后面
...
适合看当前有哪些 run 正在跑、最近几条 run 的状态分别是什么。
...
/dispatch-task codex:aliyun-codex timeout=600 wait=true -- 阻塞等待 API 层和测试都完成后再返回。
/dispatch-task codex:current monitor=pane -- 在右侧 tmux pane 启动真实 Codex 子会话执行任务。
```
...
- 想把 Codex 子任务开到右侧 pane，需要先进入 tmux，再加 `monitor=pane`
- `monitor=pane` 不能和 `wait=true` 一起用
...
- **Claude alias home**：`~/.ccswitch-pro/alias-homes/<alias>/`
- **Dispatch 状态与历史**：`~/.ccswitch-pro/dispatch-status.json`、`~/.ccswitch-pro/dispatch-history.jsonl`
