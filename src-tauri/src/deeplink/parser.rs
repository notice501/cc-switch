...
//!
//! Parses application deep-link URLs into DeepLinkImportRequest structures.
...
/// Parse an application deep-link URL into a DeepLinkImportRequest
///
/// Expected format:
/// <scheme>://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    let expected_scheme = crate::app_identity::deeplink_scheme();
    if scheme != expected_scheme {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected '{expected_scheme}', got '{scheme}'"
        )));
    }
